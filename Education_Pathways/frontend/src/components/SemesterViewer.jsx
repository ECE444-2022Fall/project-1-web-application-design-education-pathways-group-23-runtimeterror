...
class SemesterViewer extends Component {
  ...
  componentDidMount() {
    ...
    drake.on("drop", (el, target, source) => {
      API.post("/api/swap_semester", {
        course: el.id,
        source_semester: source.id - 1,
        target_semester: target.id - 1,
      }).then((res) => {
        this.setState({
  ...
  closeForm() {
    if (document.getElementById("major").value === "true") {
      document.getElementById("notification-form").innerHTML =
    ...
    API.post("/api/create_student", { major: major, year: year }).then(
      (res) => {
        this.setState({
          major: res.data.major,
          year: res.data.year,
  ...
  addCourseBox(column_id) {
    ...
    var newCourseName = document.getElementById(
      "course_code_" + column_id
    ).value;
    var isValid =
      document.getElementById(newCourseName) === null ? true : false;
    // console.log(isValid);
    if (!isValid) {
    ...
    if (coursesNumber > 6) {
      document.getElementById("notification-" + column_id).innerHTML =
        "You can add maximum 6 courses per semester.";
      return;
    }

    API.post("/api/get_course_category", { course: newCourseBox.id }).then(
      (res) => {
        if (res.status === 200) {
          newCourseBox.classList.add("course-" + res.data.category);
          newCourseBox.style.color = this.state[res.data.category];

          API.post("/api/add_course", {
            semester: column_id - 1,
            course: newCourseBox.id,
  ...
  removeCourseBox(column_id) {
    ...
    if (courseBox === null) {
      ...
      document.getElementById("notification-" + column_id).innerHTML =
        "This course is not in this semester.";
      return;
    }

    API.post("/api/remove_course", {
      semester: column_id - 1,
      course: CourseName,
    }).then((res) => {
      this.setState({
  ...
  handleColorChange = debounce((type) => {
        ...
        <div class="form-popup" id="studentForm">
          <form onSubmit={this.handleSubmit} class="form-container">
            ...
            <select id="major" name="major" required>
              ...
              <option value="Electrical & Computer">
                Electrical & Computer
              </option>
            ...
            <p className="notification" id="notification-form">
              {" "}
            </p>
            <button type="button" class="btn" onClick={this.closeForm}>
              Submit
            </button>
          </form>
        </div>
          ...
          <ul className="drag-list">
            <li className="drag-column">
              <table>
                <tr>
                  <td rowspan="2">
                    <span className="student-info-header">
                      <h2>Current Status</h2>
                    </span>
                  </td>
                  <td className="student-info">
                    Your Major is: {this.state.major}
                  </td>
                  <td className="student-info" style={{ paddingLeft: "40px" }}>
                    Earned Credits: {this.state.earned_credits}
                  </td>
                </tr>
                <tr>
                  <td className="student-info">Major Completion: Incomplete</td>
                  <td className="student-info" style={{ paddingLeft: "40px" }}>
                    Planned Credits: {this.state.planned_credits}
                  </td>
                </tr>
              </table>
            </li>
                    ...
                    <ul class="legend-labels">
                      <li>
                        <input
                          ...
                          onChange={() => this.handleColorChange("core")}
                        />
                        Core
                      </li>
                      <li>
                        <input
                          type="color"
                          id="elective-color"
                          defaultValue={this.state.elective}
                          onChange={() => this.handleColorChange("elective")}
                        />
                        Elective
                      </li>
                      <li>
                        <input
                          type="color"
                          id="minor-color"
                          defaultValue={this.state.minor}
                          onChange={() => this.handleColorChange("minor")}
                        />
                        Minor
                      </li>
                      <li>
                        <input
                          ...
                          onChange={() => this.handleColorChange("extra")}
                        />
                        Extra
                      </li>
                    </ul>
            ...
            <li className="drag-column">
              ...
              <ul className="drag-inner-list" id={1}></ul>
              <form className="add-course">
                <label htmlFor="course_code_1" className="form-input">
                  Course Code:
                </label>
                ...
                <button
                  type="button"
                  className="form-button"
                  onClick={() => this.removeCourseBox("1")}
                >
                  Remove
                </button>
              </form>
              <p className="notification" id="notification-1">
                {" "}
              </p>
            </li>
            <li className="drag-column">
              ...
              <ul className="drag-inner-list" id={2}></ul>
              <form className="add-course">
                <label htmlFor="course_code_2" className="form-input">
                  Course Code:
                </label>
                ...
                <button
                  type="button"
                  className="form-button"
                  onClick={() => this.removeCourseBox("2")}
                >
                  Remove
                </button>
              </form>
              <p className="notification" id="notification-2">
                {" "}
              </p>
            </li>
            <li className="drag-column">
              ...
              <ul className="drag-inner-list" id={3}></ul>
              <form className="add-course">
                <label htmlFor="course_code_3" className="form-input">
                  Course Code:
                </label>
                ...
                <button
                  type="button"
                  className="form-button"
                  onClick={() => this.removeCourseBox("3")}
                >
                  Remove
                </button>
              </form>
              <p className="notification" id="notification-3">
                {" "}
              </p>
            </li>
            <li className="drag-column">
              ...
              <ul className="drag-inner-list" id={4}></ul>
              <form className="add-course">
                <label htmlFor="course_code_4" className="form-input">
                  Course Code:
                </label>
                ...
                <button
                  type="button"
                  className="form-button"
                  onClick={() => this.removeCourseBox("4")}
                >
                  Remove
                </button>
              </form>
              <p className="notification" id="notification-4">
                {" "}
              </p>
            </li>
            <li className="drag-column">
              ...
              <ul className="drag-inner-list" id={5}></ul>
              <form className="add-course">
                <label htmlFor="course_code_5" className="form-input">
                  Course Code:
                </label>
                ...
                <button
                  type="button"
                  className="form-button"
                  onClick={() => this.removeCourseBox("5")}
                >
                  Remove
                </button>
              </form>
              <p className="notification" id="notification-5">
                {" "}
              </p>
            </li>
            <li className="drag-column">
              ...
              <ul className="drag-inner-list" id={6}></ul>
              <form className="add-course">
                <label htmlFor="course_code_6" className="form-input">
                  Course Code:
                </label>
                ...
                <button
                  type="button"
                  className="form-button"
                  onClick={() => this.removeCourseBox("6")}
                >
                  Remove
                </button>
              </form>
              <p className="notification" id="notification-6">
                {" "}
              </p>
            </li>
            <li className="drag-column">
              ...
              <ul className="drag-inner-list" id={7}></ul>
              <form className="add-course">
                <label htmlFor="course_code_7" className="form-input">
                  Course Code:
                </label>
                ...
                <button
                  type="button"
                  className="form-button"
                  onClick={() => this.removeCourseBox("7")}
                >
                  Remove
                </button>
              </form>
              <p className="notification" id="notification-7">
                {" "}
              </p>
            </li>
            <li className="drag-column">
              ...
              <ul className="drag-inner-list" id={8}></ul>
              <form className="add-course">
                <label htmlFor="course_code_8" className="form-input">
                  Course Code:
                </label>
                ...
                <button
                  type="button"
                  className="form-button"
                  onClick={() => this.removeCourseBox("8")}
                >
                  Remove
                </button>
              </form>
              <p className="notification" id="notification-8">
                {" "}
              </p>
            </li>
          </ul>
