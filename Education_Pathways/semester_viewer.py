...
class CreateStudent(Resource):
    def post(self):
        ...
        for i in range(year - 4, year):
            if i + 1 < date.today().year or (
                i < date.today().year and 4 < date.today().month
            ):
                status = ["complete"] * 2
            elif i > date.today().year or (
                i + 1 > date.today().year and 9 > date.today().month
            ):
                status = ["planned"] * 2
            else:
    ...
    def post(self):
        ...
        if session.get("student"):
            ...
            if config.course_collection.count_documents({"Code": course}, limit=1) != 0:
                ...
                if any(
                    course in requirement
                    for requirement in major.requirements.core_requirements
                ):
                    category = "core"
                elif any(
                    course in requirement
                    for requirement in major.requirements.elective_requirements
                ):
                    category = "elective"
...
class SwapSemester(Resource):
    def post(self):
        ...
        if session.get("student"):
            ...
            index = (
                student.get_semester(index=int(source_semester))
                .get_courses()
                .index(course)
            )
