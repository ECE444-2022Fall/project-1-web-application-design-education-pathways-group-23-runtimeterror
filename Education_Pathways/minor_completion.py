...
class CheckMinorRequirements(Resource):
    def get(self):
        ...
        if not session.get("student"):
            resp = jsonify({"error": "Student course list has not been set"})
            resp.status_code = 204
            return resp

        if minor_name == "":
            resp = jsonify({"error": "Minor has not been provided"})
            resp.status_code = 204
            return resp
        student = Student.deserialize(session["student"])

        minor_collection = config.db["minors"]
        minor_db_object = list(minor_collection.find({"name": minor_name}))

        if len(minor_db_object) == 0:
            resp = jsonify({"error": "Could not find minor in list"})
            resp.status_code = 204
            return resp
