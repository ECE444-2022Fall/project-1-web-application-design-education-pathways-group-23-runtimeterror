...
#To check whether a course taken is extra than what is needed by the minor

def extra_minor_courses(course_list):

    complete_minor_name = " "
    for minor in engineering_minor_list.keys():
        #To find out which minor is being completed
        #Assuming that there is one minor being completed

        obj = Minor(name = minor, requirements = requirements)
        if(check_minor_completion(obj,course_list)):
            complete_minor_name = obj.name
            break

    completed_minor = engineering_minor_list.get(complete_minor_name)
    #Get the list of all courses of the completed minor

    for course in completed_minor:
        if course in course_list and course not in minor_courses:
            print("These courses are extra and do not count towards your minor \n", course)
