...
def test_minor_progress(minor):
    ...
    # Check correct list
    correct_course_list = ["APS360H1", "ECE358H1",
                           "ROB311H1", "CSC311H1", "CSC485H1", "CHE322H1"]
    progress_list = minor.check_progress(correct_course_list)
    for req in progress_list:
        assert req[1] == True,\
            f"The Minor requirement {req} should be fulfilled"
...
def test_minor_completion(minor):
    ...
    assert minor.check_completion(
        correct_course_list), "This course list should fulfill minor requirements"

    # Check blank list
    incorrect_course_list1 = []
    assert not minor.check_completion(
        incorrect_course_list1), "This course list should not fulfill minor requirements"

    # Check incorrect list with course that fulfills multiple requirements
    incorrect_course_list2 = ["APS360H1", "ECE358H1",
                              "ROB311H1", "CSC311H1", "CHE507H1"]
    assert not minor.check_completion(
        incorrect_course_list2), "This course list should not fulfill minor requirements"
...
def test_major_progress(major):
    ...
    # Check correct list
    correct_course_list = ["ESC499Y1", "MIE429H1",
                           "MIE451H1", "ECE352H1", "CSC310H1", "CSC413H1"]
    progress_list = major.check_progress(correct_course_list)
    for req in progress_list:
        assert req[1] == True,\
            f"The Major requirement {req} should be fulfilled"
    ...
    for req in progress_list:
        ...
        elif req[0] == ['CSC310H1', 'CSC413H1', 'CSC401H1', 'CSC420H1', 'CSC485H1', 'CSC486H1', 'MIE424H1', 'MIE457H1', 'MIE566H1', 'ECE352H1', 'ECE419H1']:
            assert req[1] == False,\
                f"The Major requirement {req} should not be fulfilled"
        else:
            assert req[1] == True,\
...
def test_major_completion(major):
    ...
    assert major.check_completion(
        correct_course_list), "This course list should fulfill major requirements"

    correct_course_list = ["ESC499Y1", "MIE429H1",
                           "MIE451H1", "ECE352H1", "ECE419H1", "CSC413H1"]
    assert major.check_completion(correct_course_list), "This course list should fulfill major requirements" \
        + "using core courses as electives"

    # Check blank list
    incorrect_course_list1 = []
    assert not major.check_completion(
        incorrect_course_list1), "This course list should not fulfill major requirements"
