...
def test_course_prereq_api(client):
    with client:
        ...
        # Check the same number of Pre-reqs
        assert len(course['prereq']) == len(real_pre_req)
        for i in real_pre_req:
            # Check that all pre-reqs match perfect
            assert i in course['prereq']
...
def test_course_prereq_with_student_api(client):
    with client:
        ...
        year = 2023
        course = "ECE243H1"
        response = client.post("/api/create_student",
                               json={"major": major, "year": year})
        response = client.get("/api/load_student")
        response = client.post(
            "/api/add_course", json={"course": course, "semester": 0, "category": "elective"})
        assert response.status_code == 200

        student = Student.deserialize(session.get("student"))
        # Test

        # Check student created properly
        ...
        assert len(course['prereq']) == len(
            real_pre_req)  # Check pre-req length matches
        assert len(course['takenreq']) == len(real_pre_req) - \
            1  # Check correct number of taken reqs
        # Check that it populates correctly from student
        assert course['takenreq'] == ["ECE243H1"]
        # Check that it exists in student courses
        assert "ECE243H1" in student.get_courses()
        assert course['code'] == "ECE344H1"  # Finally re-check the course code
