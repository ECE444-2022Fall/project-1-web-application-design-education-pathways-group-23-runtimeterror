...
def student():
    ...
    return Student(major=major, year=year, minors=minors, semesters=semesters)
...
def test_student_get_courses(student):
    ...
    assert (
        student.get_courses(status=["planned"]) == []
...
def test_student_course_swap(student):
    ...
    assert not "ECE568H1" in student.get_semester(
        "Fall 2022"
    ), "Course should not be in this Semester"
    assert "ECE568H1" in student.get_semester(
        "Winter 2021"
...
def test_check_major_status():
    ...
    elective_requirements = [["ECE444H1"], ["ECE419H1"]]
