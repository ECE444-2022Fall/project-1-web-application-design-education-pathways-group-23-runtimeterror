...
complete_student.add_semester(
    "Fall 2019",
    "complete",
    ["APS360H1", "ECE345H1", "CSC384H1", "ECE421H1", "ECE368H1", "ECE344H1"],
)
...
def test_get_minor_completion_fail_when_no_student(client):
    with client:
        ...
        response = client.post(
            "/api/get_minor_completion", json={"minor_name": minor_name}
        )
...
def test_get_ai_minor_completion_with_partially_incomplete_student(client):
    ...
    with client:
        ...
        assert course_completion_status, "The course completion response isn't provided"
        for i in range(len(expected)):
            assert (
                course_completion_status[i] == expected[i]
