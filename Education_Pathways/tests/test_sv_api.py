...
def test_create_student(client):
    with client:
        ...
        response = client.post(
            "/api/create_student", json={"major": major, "year": year}
        )
...
def test_get_course_category(client):
    with client:
        ...
        response = client.post(
            "/api/get_course_category", json={"course": "Fake Course"}
        )
        assert response.status_code == 204
...
def test_add_remove_course(client):
    with client:
        ...
        response = client.post(
            "/api/add_course",
            json={"course": course, "semester": semester, "category": "elective"},
        )
        ...
        response = client.post(
            "/api/remove_course", json={"course": course, "semester": 0}
        )
...
def test_get_course_categories(client):
    with client:
        ...
        for i in range(4):
            response = client.post(
                "/api/get_course_category", json={"course": courses[i]}
            )
        ...
        response = client.post(
            "/api/remove_course",
            json={"course": courses[2], "semester": source_semester},
        )
        response = client.post(
            "/api/add_course",
            json={
